...
import Svg, { Circle, Path, Defs, LinearGradient, Stop, Polygon, Rect } from 'react-native-svg';
import { colors } from '@/presentation/themes/colors';
const { width } = Dimensions.get('window');
const DoctorIllustration = () => (<Svg width={140} height={140} viewBox="0 0 140 140">
    <Defs>
...
const AuthBackgroundHUD = () => {
    ...
    useEffect(() => {
        ...
        drift1.value = withRepeat(withSequence(withTiming(1, { duration: 4000, easing: Easing.inOut(Easing.sin) }), withTiming(0, { duration: 4000, easing: Easing.inOut(Easing.sin) })), -1, true);
        // Drifting motion for right nodes (Offset timing)
        drift2.value = withRepeat(withSequence(withTiming(1, { duration: 5000, easing: Easing.inOut(Easing.sin) }), withTiming(0, { duration: 5000, easing: Easing.inOut(Easing.sin) })), -1, true);
    }, []);
