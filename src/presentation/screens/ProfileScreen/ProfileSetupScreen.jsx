...
import { View, Text, Alert, ScrollView, StyleSheet, Pressable, TextInput, } from 'react-native';
...
import Button from '@/presentation/common/ui/Button';
...
function InputField({ label, value, onChangeText, placeholder, icon, keyboardType = 'default', suffix }) {
    const [focused, setFocused] = useState(false);
    return (<View style={fieldStyles.wrapper}>
      <Text style={fieldStyles.label}>{label}</Text>
      <View style={[fieldStyles.row, focused && fieldStyles.rowFocused]}>
        <MaterialIcons name={icon} size={18} color={focused ? colors.primary.main : colors.text.muted}/>
        <TextInput style={fieldStyles.input} value={value} onChangeText={onChangeText} placeholder={placeholder} placeholderTextColor={colors.text.muted} keyboardType={keyboardType} onFocus={() => setFocused(true)} onBlur={() => setFocused(false)}/>
...
const ProfileSetupScreen = () => {
    ...
    const [gender, setGender] = useState(isEditMode && user?.gender ? user.gender : 'male');
    ...
    const handleSave = async () => {
        ...
        if (validation !== true) {
            Alert.alert('Incomplete Profile', validation);
            return;
        }
        setIsLoading(true);
        await saveProfile();
    ...
    return (<SafeAreaWrapper>
      {isEditMode && (<View style={styles.topNav}>
      ...
      <ScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
        <View style={styles.header}>
          {!isEditMode && (<View style={styles.stepBadge}>
        ...
        {!isEditMode && (<View style={styles.progressBar}>
        ...
        <View style={styles.form}>
          ...
          <View style={styles.formGroup}>
            ...
            <View style={styles.genderRow}>
              {GENDER_OPTIONS.map((opt) => (<Pressable key={opt.value} style={[styles.genderChip, gender === opt.value && styles.genderChipSelected]} onPress={() => setGender(opt.value)}>
        ...
        {/* CTA */}
        <Button title={isEditMode ? "Save Changes" : "Continue to Dashboard"} onPress={handleSave} loading={isLoading} icon={isEditMode ? "save" : "arrow-forward"} disabled={!name || !age || !weight || !height}/>
      </ScrollView>
    </SafeAreaWrapper>);
};
