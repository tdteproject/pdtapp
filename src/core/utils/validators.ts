import { AuthFormData, User } from '@/core/types';
...
export const validateAuthForm = (data: AuthFormData): string | true => {
  const phoneErr = validatePhone(data.phone);
  if (phoneErr !== true) return phoneErr;
  const otpErr = validateOTP(data.otp);
  if (otpErr !== true) return otpErr;
  return true;
};

export const validateProfile = (user: Partial<User>): string | true => {
